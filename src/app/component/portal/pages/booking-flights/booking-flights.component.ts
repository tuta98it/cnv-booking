...
export class BookingFlightsComponent extends TableSelectionAbstract implements OnInit, OnDestroy {
    ...
    this.generalService.getAdminticket(payload).subscribe((res: any) => {
      if (res !== null) {
        ...
        this.datas.forEach(en => {
          ...
          en.dateCreated = this.dateFormatPipe.transformFull(en.dateCreated);
          // en.status = en.status == 'OK' ? 'Xem vé' : en.status == 'FAIL' ? 'Chưa có vé' : '';
        });
